const move = (mat: number[][], p: number, q: number) => {
  let Xmoves: number[] = [2, -1, 1, -2, -2, -1, 1, 2];
  let Ymoves: number[] = [-1, 2, 2, 1, -1, -2, -2, 1];
  let validMoves: number[][] = [];

  for (let i: number = 0; i < 8; i++) {
    let x: any = p + Xmoves[i];
    let y: any = q + Ymoves[i];
...
const prob = (mat: number[][], pos: number[], turns: number) => {
  let valData: number[] = [0, 0];
...
let res = prob(board, position, turns);
