...
const mainFunc = (points: number[][], centPoint: number[], k: number): number[][] => {
  let res: number[][] = points.sort((p1: number[], p2: number[]) => dist(p1, centPoint) - dist(p2, centPoint)).slice(0, k);
